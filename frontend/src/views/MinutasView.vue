  ...
  <div class="huella-page">
    ...
    <template v-else-if="agg">
      <section class="kpis" style="margin-bottom:26px">
        ...
        <div class="card kpi"><div class="v">{{ agg.kpis.origenes_documentados }}</div><div class="l">orígenes documentados</div></div>
      ...
      <section class="card" style="margin-top:22px">
        ...
        <table>
          <thead><tr><th>Clave</th><th>Denominación</th><th>Origen</th><th>ODS / metas</th><th>Confianza</th></tr></thead>
          <tbody>
            <tr v-for="m in lista" :key="m.id">
              <td>{{ m.clave }}</td>
              <td>{{ m.denominacion }}<br><span class="muted">{{ m.tema }}</span></td>
              <td>
                <span v-if="m.origen">{{ m.origen }}</span>
...
function clearFilters() { q.value = ''; fOrigen.value = ''; fOds.value = ''; fMeta.value = ''; loadLista(); }
